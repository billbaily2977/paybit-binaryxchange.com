...
if (isset($_POST['paid']) || isset($_POST['paida'])) {
    ...
    if ($amount < 10) {
        ...
        if (!in_array(strtolower($ext), $allowed)) {
            $msg = "Only JPG, PNG, PDF allowed";
        } elseif (move_uploaded_file($_FILES['file']['tmp_name'], $filePath)) {
            
            // 1. Insert deposit request
            $stmt = $pdo->prepare("INSERT INTO deposits (user_id, amount, proof_path, status) VALUES (?, ?, 'pending')");
            $stmt->execute([$userId, $amount, $filePath]);

            // 2. Update total_deposit for tracking
            $pdo->prepare("UPDATE users SET total_deposit = total_deposit + ? WHERE id = ?")
                ->execute([$amount, $userId]);

            $msg = "Deposit request of $$amount submitted. Wait for approval.";
        } else {
...
<title>PayBit-BinaryXchange | Payment</title>
        ...
        <div class="col-sm-16 col-md-8">
                <div class="card">
                        <div class="card-header align-items-start justify-content-between flex">
                                <h4 class="pull-left">Bitcoin Payment Details</h4>
                                <ul class="nav nav-pills card-header-pills pull-right">
                                        <li class="nav-item">
                                                <button class="btn btn-sm btn-link btn-round" data-toggle="collapse" data-target="#bpd"><i class="fa fa-chevron-down"></i></button>
                                        </li>
                                </ul>
                        </div>
                        <div id="bpd">
                                <div class="card-body">
                                        <div class="form-group" align="center">
                                                ...
                                                <hr>
                                                <img src="https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl=bitcoin:bc1qqu70gcgd0gk7naf7y5ncauvm29arfzf50t6qp2" style="width:200px;">
                                                <hr style="color:white">
                                                <div class="input-group mb-3" style="width:80%">
                                                        <input style="color:black" id="myInput" type="text" value="16SH6P6M4wVzUvhGbZBu35vH74bhYcuA4t" class="form-control" readonly>
                                ...
                                <form action="" method="POST" enctype="multipart/form-data">
                                        <div class="form-group" style="border: 0px solid grey;border-radius:5px;padding:15px;">
                                                <p class="form-control-label font-weight-bold text-center" style="color:#17a2b8;font-size:12px">
                                                ...
                                                <button type="submit" name="paid" style="float:right" class="btn btn-primary">Submit Payment Proof</button>
                                        </div>
                                </form>
                        </div>
                </div>
                ...
                <div class="card">
                        <div class="card-header align-items-start justify-content-between flex">
                                <h4 class="pull-left">Ethereum(ERC20) Payment Details</h4>
                                <ul class="nav nav-pills card-header-pills pull-right">
                                        <li class="nav-item">
                                                <button class="btn btn-sm btn-link btn-round" data-toggle="collapse" data-target="#epd"><i class="fa fa-chevron-down"></i></button>
                                        </li>
                                </ul>
                        </div>
                        <div id="epd">
                                <div class="card-body">
                                        <div class="form-group" align="center">
                                                <label for="recipient-name" class="form-control-label font-weight-bold">Scan Ethereum(ERC20) QR Code to copy wallet address</label>
                                                <hr>
                                                <img src="https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl=ethereum:0x8A301F2b0dB4C470EffdaAB4244e0cF6251b2454" style="width:200px;">
                                                <hr style="color:white">
                                                <div class="input-group mb-3" style="width:80%">
                                                        <input style="color:black" id="myInputa" type="text" value="0x625ec49be4fb8056ef2e64c993a0d0b84373f17a" class="form-control" readonly>
                                                        <div class="input-group-append">
                                ...
                                <form action="" method="POST" enctype="multipart/form-data">
                                        <div class="form-group" style="border: 0px solid grey;border-radius:5px;padding:15px;">
                                                ...
                                                <hr>
                                                <button type="reset" class="btn btn-secondary" data-dismiss="modal">Clear</button>
                                                <button type="submit" name="paida" style="float:right" class="btn btn-primary">Submit Payment Proof</button>
                                        </div>
                                </form>
                        </div>
                </div>
...
<script>
        ...
        function copyWalleta() {
                ...
                copyText.setSelectionRange(0, 99999);
                document.execCommand("copy");
                Swal.fire('', '<b>Ethereum(ERC20) Wallet Copied : </b> ' + copyText.value + '', '');
        }
</script><br><br>
